...
function createSuggestionIcon(stop) {
  ...
  const isHotel = stop.category === 'hotel'
  let src, size
  if (slug === 'bib-gourmand')    { src = bibsUrl;     size = 11 }
  else if (slug.includes('star')) { src = etoileUrl;   size = 10 }
  else if (isHotel)               { src = litUrl;      size = 10 }
  else                            { src = couvertsUrl; size = 9  }
  const W = 26, H = 34
  const top  = Math.round(3 + (18 - size) / 2)
  const left = Math.round((W - size) / 2)
  return L.divIcon({
    html: `<div style="position:relative;width:${W}px;height:${H}px;opacity:0.82;">
      <svg viewBox="0 0 32 42" width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
        <path d="M16,0C7.163,0,0,7.163,0,16C0,24.837,16,42,16,42s16-17.163,16-26C32,7.163,24.837,0,16,0z"
          fill="white" stroke="#aaaaaa" stroke-width="1.5"/>
        <circle cx="16" cy="15" r="11" fill="#888888"/>
      </svg>
      <img src="${src}" width="${size}" height="${size}"
        style="position:absolute;top:${top}px;left:${left}px;filter:brightness(0) invert(1);pointer-events:none;"
      />
...
export default function RoadtripPlannerMap({
  origin,
  destination,
  polyline,
  suggestions,
  ...
  const selectedSet = new Set(selectedStops.map((s) => `${s.category}:${s.id}`))

  const allPoints = [
    ...(origin?.lat != null      ? [[origin.lat, origin.lng]]           : []),
    ...(destination?.lat != null ? [[destination.lat, destination.lng]] : []),
    ...selectedStops.filter((s) => s.lat != null).map((s) => [s.lat, s.lng]),
    ...suggestions.filter((s) => s.lat != null && !selectedSet.has(`${s.category}:${s.id}`)).map((s) => [s.lat, s.lng]),
  ]
  ...
  return (
    <MapContainer center={[46.6, 2.2]} zoom={6} className={styles.map}>
      ...
      {suggestions.map((stop) => {
        if (selectedSet.has(`${stop.category}:${stop.id}`)) return null
        const dist = formatDistinction(stop.distinction_slug)
        return (
          <Marker
            key={`sug-${stop.category}-${stop.id}`}
            position={[stop.lat, stop.lng]}
            icon={createSuggestionIcon(stop)}
          >
            <Popup>
              <div className={styles.popup}>
                {stop.image && (
                ...
                <p className={styles.popupType}>{stop.category === 'restaurant' ? 'Restaurant' : 'Hôtel'}</p>
                ...
                <button className={styles.popupAddBtn} onClick={() => onAdd(stop)}>
                  + Ajouter ce stop
                </button>
              </div>
            </Popup>
      ...
      {selectedStops.map((stop) => {
        ...
        return (
          <Marker
            key={`sel-${stop.category}-${stop.id}`}
            position={[stop.lat, stop.lng]}
            icon={createSelectedIcon(stop)}
          >
            <Popup>
              <div className={styles.popup}>
                {stop.image && (
                ...
                {stop.city && <p className={styles.popupCity}>{stop.city}</p>}
                {dist && <p className={styles.popupDist}>{dist}</p>}
                <button className={styles.popupRemoveBtn} onClick={() => onRemove(stop.category, stop.id)}>
                  Retirer ce stop
                </button>
              </div>
            </Popup>
