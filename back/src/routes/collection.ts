...
function parseRegionName(raw: string | null): string {
  if (!raw) return 'Autre';
  const trimmed = raw.trim();
  if (trimmed.startsWith('{')) {
    try { return (JSON.parse(trimmed) as Record<string, string>).name ?? trimmed; }
...
router.get('/continents', async (_req, res, next) => {
  try {
    ...
    const continentMap: Record<string, { total: number; visited: number }> = {};
...
router.get('/continents/:continent/countries', async (req, res, next) => {
  try {
    ...
    res.json(totalRes.rows.map((row) => ({
      code:    row.code,
...
router.get('/countries/:country/regions/:region/cities/:city/restaurants', async (req, res, next) => {
  try {
    ...
    const result = await pool.query(`
      SELECT
        r.id, r.identifier, r.name, r.distinction_score, r.distinction, r.cuisines,
        CASE WHEN col.id IS NOT NULL THEN true ELSE false END AS visited
      FROM restaurants r
      LEFT JOIN collection col ON col.id_restaurant = r.id AND col.id_client = $1
    ...
    res.json(result.rows.map((r) => {
      ...
      const dist     = (r.distinction ?? {}) as Record<string, unknown>;
      const cuisines = Array.isArray(r.cuisines) ? r.cuisines : [];
      const first    = cuisines[0] as Record<string, unknown> | string | undefined;
      const cuisine  = typeof first === 'object' && first ? String(first.label ?? '') : String(first ?? '');
      return {
        id:      String(r.identifier ?? r.id),
        name:    String(r.name ?? ''),
        stars:   Math.min(stars, 3),
        bib:     dist.slug === 'bib-gourmand',
        cuisine,
